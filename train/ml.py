...
class DomainBayesWeighter:
    ...
    def update_and_get_tau_per_step(self, grads_history, step_domains):
        ...
        norms = torch.stack([f.norm() if f.numel() > 0 else torch.tensor(0.0, device=device) for f in flat])
        ...
        with torch.no_grad():
            for s in range(n_steps):
                ...
                new_var = self.var.index_select(0, idx) * self.ema + (1 - self.ema) * delta * (v - new_mean)
...
def _compute_probabilistic_arith_weights(grads_history, beta=0.5, _weight_noise_state=None,
                                         target_sum=1.0, bayes_weighter=None, device=None,
                                         step_domains=None):
    n = len(grads_history)
    if n == 0:
...
def accumulate_meta_grads_arith_prob(
        net, grads_history, meta_lr, scaled_factor=3.0, bayes_ema=0.9,
        beta=0.5, _bayes_state=dict(), step_domains=None, domain_count=None,
        learnable_domain_scale=False, domain_scale_lr=1e-3,
):
    ...
    w = _compute_probabilistic_arith_weights(
        ...
        bayes_weighter=bw, step_domains=step_domains, device=device,
    )
